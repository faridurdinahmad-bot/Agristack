...
@section('title', 'Supplier Groups')
@section('page-title', 'Supplier Groups')

@section('content')
<div class="space-y-6">
    {{-- Page Header: Title + Add button + Search --}}
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <h2 class="text-lg font-semibold text-slate-900 dark:text-white">Supplier Groups</h2>
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
            <label for="supplier-group-search" class="sr-only">Search by group name</label>
            <input type="text"
                   id="supplier-group-search"
    ...
    <div class="rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 overflow-hidden">
            ...
            <table class="w-full">
                <thead class="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-center text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Total Suppliers</th>
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-slate-700" id="supplier-groups-table-body">
                    @php
                        $sampleGroups = [
                            ['name' => 'Agriculture & Seeds', 'description' => 'Suppliers of seeds, fertilizers, and farm inputs', 'totalSuppliers' => 12, 'status' => 'active'],
                            ['name' => 'Hardware & Tools', 'description' => 'Industrial and construction supplies', 'totalSuppliers' => 8, 'status' => 'active'],
                            ['name' => 'Livestock Feed', 'description' => 'Animal feed and related products', 'totalSuppliers' => 5, 'status' => 'active'],
                            ['name' => 'Pesticides & Chemicals', 'description' => 'Crop protection and chemical suppliers', 'totalSuppliers' => 4, 'status' => 'active'],
                            ['name' => 'Legacy / Inactive', 'description' => 'Old group no longer in use', 'totalSuppliers' => 0, 'status' => 'inactive'],
                        ];
                    @endphp
                    @foreach ($sampleGroups as $index => $group)
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                            ...
                            <td class="px-4 py-3 text-center">
                                <span class="text-sm text-slate-900 dark:text-slate-100">{{ $group['totalSuppliers'] }}</span>
                            </td>
                            <td class="px-4 py-3 text-center">
                            ...
                            <td class="px-4 py-3 whitespace-nowrap text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <button type="button" class="supplier-group-view px-3 py-1.5 rounded-lg text-xs font-medium text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-700/50 hover:bg-slate-200 dark:hover:bg-slate-600/50 transition-colors" title="View">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                        </svg>
                                    </button>
                                    <button type="button" class="supplier-group-edit px-3 py-1.5 rounded-lg text-xs font-medium text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-700/50 hover:bg-slate-200 dark:hover:bg-slate-600/50 transition-colors inline-flex items-center justify-center" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                        </svg>
                                    </button>
                                    <button type="button" class="supplier-group-delete px-3 py-1.5 rounded-lg text-xs font-medium text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-500/10 hover:bg-red-100 dark:hover:bg-red-500/20 transition-colors" title="Delete">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div id="empty-state" class="hidden p-12 text-center">
            <div class="max-w-md mx-auto">
                ...
                <h3 class="text-lg font-semibold text-slate-900 dark:text-white mb-2">No supplier groups found</h3>
                <p class="text-sm text-slate-500 dark:text-slate-400 mb-6">Get started by creating your first supplier group.</p>
...
{{-- Add / Edit Supplier Group Modal (UI only) --}}
<div id="supplier-group-modal" class="fixed inset-0 z-50 hidden" aria-hidden="true">
    ...
    <div class="fixed inset-0 flex items-center justify-center p-4">
        <div class="w-full max-w-md rounded-2xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 shadow-xl" role="dialog" aria-labelledby="modal-title" aria-modal="true">
            <div class="p-6">
                <h3 id="modal-title" class="text-lg font-semibold text-slate-900 dark:text-white mb-5">Add Supplier Group</h3>
                <form id="supplier-group-form" class="space-y-5">
                    <div>
                        ...
                        <input type="text"
                               ...
                               name="group_name"
                               required
                               placeholder="e.g. Agriculture & Seeds"
...
<script>
(function() {
    var modal = document.getElementById('supplier-group-modal');
    var openBtn = document.getElementById('open-add-modal');
    var cancelBtn = document.getElementById('modal-cancel');
    var backdrop = document.getElementById('modal-backdrop');
    var form = document.getElementById('supplier-group-form');

    function openModal() { modal.classList.remove('hidden'); }
    function closeModal() { modal.classList.add('hidden'); }

    if (openBtn) openBtn.addEventListener('click', openModal);
    document.querySelectorAll('.open-add-modal, .supplier-group-edit').forEach(function(btn) { btn.addEventListener('click', openModal); });
    ...
    // Search (UI only)
    var searchInput = document.getElementById('supplier-group-search');
    var tableBody = document.getElementById('supplier-groups-table-body');
